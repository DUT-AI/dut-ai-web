...
export default function PostLayoutAPI({ post, children }: LayoutProps) {
    const { title, created_at, updated_at, keywords, authors, slug: postSlug, views, related_blogs } = post
    const basePath = 'blog'
    const relatedPosts = related_blogs || []
                ...
                <article className="mx-auto max-w-7xl px-4 sm:px-6 lg:px-8">
                    ...
                    <header className="mb-12 mt-28">
                        ...
                        <div className="flex flex-wrap items-center gap-y-6 gap-x-10">
                            ...
                            {/* Tác giả */}
                            {authors?.map((author) => (
                                <div className="flex items-center gap-3" key={author.id}>
                                    {author.avatar_url ? (
                            ...
                            <div className="flex items-center gap-3">
                                ...
                                <div>
                                    ...
                                    <time dateTime={created_at} className="text-[15px] text-gray-900 dark:text-white font-extrabold">{new Date(created_at).toLocaleDateString('en-GB')}</time>
                                </div>
                            </div>
                            ...
                            <div className="flex items-center gap-3">
                                ...
                                <div>
                                    ...
                                    <time dateTime={updated_at || created_at} className="text-[15px] text-gray-900 dark:text-white font-extrabold">{new Date(updated_at || created_at).toLocaleDateString('en-GB')}</time>
                                </div>
                            </div>
                            ...
                            <div className="flex items-center gap-3">
                                ...
                                <div>
                                    ...
                                    <span className="text-[15px] text-gray-900 dark:text-white font-extrabold">{(views || 0).toLocaleString()}</span>
                                </div>
                            </div>

                            {/* Group Actions */}
                            <div className="flex items-center gap-2 ml-auto">
                                <SocialShare url={`${siteMetadata.siteUrl}/blog/${postSlug}`} />
                            </div>
                        </div>
                    </header>
                        ...
                        <div className="flex-1 w-full min-w-0">
                            ...
                            {keywords && keywords.length > 0 && (
                                <div className="flex flex-wrap gap-2.5 mb-6">
                                    {keywords.map((kw) => (
                                        <Link key={kw.id} href={`/blog?tag=${slug(kw.keyword_name)}`} className="bg-white dark:bg-gray-800 border border-gray-100 dark:border-gray-700 text-blue-600 dark:text-blue-400 px-5 py-2.5 rounded-full text-[13px] font-bold tracking-wide shadow-sm hover:ring-1 hover:ring-gray-200 transition-all">
                                            <span className="opacity-60 mr-1">#</span>{kw.keyword_name}
                                        </Link>
                                    ))}
                                </div>
                            )}
                            ...
                            <div className="bg-white dark:bg-gray-900 rounded-[32px] p-8 sm:p-12 lg:p-16 shadow-sm ring-1 ring-gray-100 dark:ring-gray-800">
                                ...
                                {siteMetadata.comments?.provider && (
                                    <div className="pt-10 pb-6 mt-8 border-t border-gray-100 dark:border-gray-800" id="comment">
                                        <Comments slug={postSlug || String(post.id)} />
                                    </div>
                                )}
