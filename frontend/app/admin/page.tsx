...
export default async function AdminDashboardPage() {
  const [blogs, projects, generations, events, posts, keywords] = await Promise.all([
    getBlogsQuery().catch(() => [] as any[]),
    getProjectsQuery().catch(() => [] as any[]),
    getGenerationAlbumsQuery({ publishedOnly: false }).catch(() => [] as any[]),
    getPublicEventsQuery().catch(() => [] as any[]),
    getPostsQuery().catch(() => [] as any[]),
    getBlogKeywordsQuery().catch(() => [] as any[]),
  ])

  const totalViews = (blogs as any[]).reduce(
    (acc: number, b: any) => acc + (Number(b.views) || 0),
    0
  )
  ...
  const STATS: {
    ...
    {
      title: 'Sự kiện & Moments',
      value: events.length + posts.length,
      icon: Calendar,
      color: 'text-pink-600',
      ...
      <div className="grid gap-5 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-5">
        {STATS.map((stat) => {
          ...
          return (
            <Card
              key={stat.title}
              className="relative overflow-hidden border-slate-200/80 dark:border-white/10"
            >
              <CardContent className="p-6">
                ...
                <div className="mt-4">
                  ...
                  <p className="mt-1 text-xs font-semibold tracking-wider text-slate-500 uppercase dark:text-slate-400">
                    {stat.title}
                  </p>
                </div>
              </CardContent>
                ...
                >
                  <div className="min-w-0 pr-4">
                    ...
                    <p className="mt-0.5 text-xs text-slate-400">
                      Slug: <code className="text-blue-500">{blog.slug || `#${blog.id}`}</code> •{' '}
                      {blog.views} lượt xem
                    </p>
                  </div>
