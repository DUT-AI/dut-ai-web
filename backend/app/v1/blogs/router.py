...
from .service import BlogService, KeywordService
...
def get_keywords(service: KeywordService = Depends(get_keyword_service)):
    return service.get_all()


@keyword_router.get("/{id}", response_model=KeywordResponse)
def get_keyword(id: int, service: KeywordService = Depends(get_keyword_service)):
    return service.get_by_id(id)


@keyword_router.post("/", response_model=KeywordResponse)
def create_keyword(
    keyword: KeywordCreate, service: KeywordService = Depends(get_keyword_service)
):
    return service.create(keyword)
...
def update_keyword(
    ...
    return service.update(id, keyword)


@keyword_router.delete("/{id}")
def delete_keyword(id: int, service: KeywordService = Depends(get_keyword_service)):
    service.delete(id)
    return {"status": "ok"}
